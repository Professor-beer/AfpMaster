    ...
    # @param debug - flag for debug information
        ...
        def next_process_step(self):
            ...
            if self.process_step_index is None:
                ...
                if self.postprocess and self.edit_value:
                    ...
                    funct = {"Afp_intString": Afp_intString, "Afp_floatString": Afp_floatString, "AfpFaktura_colonFloat": AfpFaktura_colonFloat,  "AfpFaktura_colonInt": AfpFaktura_colonInt}
                    local = locals()
                    ...
                    #print ("AfpFaScreen_EditLinePlugIn.next_process_step post exe:", self.edit_col, value, string)
            ...
            while loop:
                ...
                if self.process_step_index < len(self.process):
                    ...
                    #print("AfpFaScreen_EditLinePlugIn.next_process_step process:", process)
                    if process[0] == "choose":
       ...
       ## select data for gridrow due to the 'right arrow' key
        ...
        def set_initial_row(self, row):
            #print ("AfpFaScreen_EditLinePlugIn.set_initial_row row:", row)
            if Afp_isString(row):
...
class AfpFaScreen(AfpEditScreen):
    ...
    def InitWx_static(self):
        ...
        self.combo_Filter.SetSelection(AfpFaktura_inFilterList("Rechnung"))
    ...
    ## Eventhandler TextBox - get focus textbox 'ZahlBetrag'
    def On_ZahlBetrag(self, event):
        print ("AfpFaScreen.On_ZahlBetrag not yet implemented!")
    ...
    def invoke_custom_select(self):
        ...
        if Ok and data:
            if Ok == True:
                ...
                if Ok == "Suchen":
                    table, filter = AfpFaktura_possibleKinds(data)
                    where = None
    ...
    def edit_data(self, rowNr = None):
        #print("AfpFaScreen.edit_data invoked", rowNr, self.edit_typ)
        ident = None
        name = None
        text = [None, None]
        anz = [None, None]
   ...
   ## store data - overwritten from AfpEditScreen
    ...
    def expand_to_row(self, value):
        ...
        anz, ok = AfpReq_Text("Bitte Anzahl für den Artikel",value + " " + bez + " eingeben!", "", "Anzahl")
        if ok and anz:
            anz = Afp_fromString(anz)
            if not Afp_isNumeric(anz):
                anz = 1
            einzel = artikel.get_value("Nettopreis")
            preis = anz*einzel
            gewinn = preis - anz*artikel.get_value("Einkaufspreis")
            if gewinn < 0:
                gewinn = 0.0
            return [None, value, bez, Afp_toString(anz), Afp_toString(einzel), Afp_toString(preis), Afp_toString(gewinn)]
